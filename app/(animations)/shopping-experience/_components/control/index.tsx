...
export default function Control({
  ...
  return (
    <div className="w-[340px] relative z-[1]" ref={constraintsRef}>
      <motion.div
        className="w-[40px] h-[40px] relative z-[5] flex justify-center items-center rounded-full hover:cursor-grab active:cursor-grabbing"
        drag="x"
        style={{ x: controlX, backgroundColor: primaryColor }}
      ...
      {options.map((option, index) => {
        return (
          ...
          >
            <motion.div
              style={{ backgroundColor: accentColor }}
              className="absolute w-[20px] h-[20px]  rounded-full cursor-pointer"
              onClick={(event) => handleDivisionPointerDown(index, event)}
            />
            <motion.div
              style={{ color: primaryColor }}
              className="absolute h-[20px] w-max left-1/2 -translate-x-1/2 top-[120%] cursor-pointer"
              onClick={(event) => handleDivisionPointerDown(index, event)}
            >
              {option.label}
            </motion.div>
